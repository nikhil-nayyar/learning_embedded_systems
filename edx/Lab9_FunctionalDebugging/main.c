...
#define GPIO_PORTF_DATA_SW0 (*(volatile unsigned long *)0x40025040)
#define GPIO_PORTF_DATA_SW1 (*(volatile unsigned long *)0x40025004)
#define GPIO_PORTF_DATA_LED (*(volatile unsigned long *)0x40025020)
...
int main(void){  unsigned long i,last,now;
  ...
  EnableInterrupts();           // enable interrupts for the grader
	Capture = GPIO_PORTF_DATA_R;  // Initialize Capture
  while(1){
		Switch1 = GPIO_PORTF_DATA_SW0;  // Switch 1 is PF4
		Switch2 = GPIO_PORTF_DATA_SW1; // Switch 2 is PF0
		if( !Switch1 || !Switch2 ){
			Led = GPIO_PORTF_DATA_LED;   // read previous
			GPIO_PORTF_DATA_LED = GPIO_PORTF_DATA_LED^0xFF; // toggle red LED
			Delay();
		}
		if(i<50 && ((Capture - GPIO_PORTF_DATA_R)!=0)){
			now = NVIC_ST_CURRENT_R;
			Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
			Data[i] = GPIO_PORTF_DATA_R; // record PF0,3,4
			last = now;
			i++;
		}
		Capture = GPIO_PORTF_DATA_R;		
  }
}
