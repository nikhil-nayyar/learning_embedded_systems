...
#define GPIO_PORTF_RED          (*((volatile unsigned long *)0x40025008))
#define GPIO_PORTF_GREEN        (*((volatile unsigned long *)0x40025020))
...
// Outputs: None
void WaitForASLow(void){
	unsigned long AS = GPIO_PORTF_SWITCH >> 4;
	while(AS){
		// Wait while AS is high, repeatedly poll AS
			AS = GPIO_PORTF_SWITCH;
	}	
	return;	
}
...
// write this function
	unsigned long AS = GPIO_PORTF_SWITCH >> 4;
	while(!AS){
		// Wait while AS is LOW, repeatedly poll AS
			AS = GPIO_PORTF_SWITCH;
	}
	return;
}
