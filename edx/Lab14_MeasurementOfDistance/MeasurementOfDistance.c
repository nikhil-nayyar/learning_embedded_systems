...
unsigned static long Flag;       // 1 means valid Distance, 0 means Distance is empty
unsigned static long adc_count;
...
int main(void){ 
  ...
  ADC0_Init();   // initialize ADC0, channel 1, sequencer 3
	lcd_general_init(); // Initialize Screen
  delay = 0;  //    wait for clock to stabilize
  while(1){ 
	
	// read mailbox if data present
	if(Flag){
		UART_ConvertDistance(ADCdata); // generate data in string form
		lcd_write_row1(String); 	
		Flag = 0; // Clear Flag
	}
